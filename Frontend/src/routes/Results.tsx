  ...
  const filteredAndSortedMatches = useMemo(() => {
    ...
    if (roleFilter.length > 0) {
      filtered = filtered.filter((match) =>
        match.candidate.roles.some((role) => roleFilter.includes(role))
      );
    }
    ...
    const sorted = [...filtered].sort((a, b) => {
      switch (sortBy) {
        ...
        case "skills":
          return b.candidate.skills.length - a.candidate.skills.length;
        case "interests":
          return b.candidate.interests.length - a.candidate.interests.length;
        default:
          return 0;
      }
    });
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            {filteredAndSortedMatches.length} teammate
            {filteredAndSortedMatches.length !== 1 ? "s" : ""} found
          </p>
        </div>
